...
### N.B. this version only works with a single task!!! 
...
class SEM(object):
    ...
    def update_single_event(self, x, update=True, save_x_hat=False):
        ...
        log_like[-1, :len(active)] = np.min(lik[[1, 3, 4],:], axis=0)  # only look at the relevant scenes for the task (NOTE: this is hard
            # and only works for the schema prediction task)
