...
declare -a learn_rate_arr=(0.01 0.05 0.1)
declare -a condition_arr=("blocked" "interleaved")
declare -a alfa_arr=(0.01 0.1 1 10 100 1000 10000 100000)
declare -a lmda_arr=(0.01 0.1 1 10 100 1000 10000 100000)
...
for learn_rate in "${learn_rate_arr[@]}"; do
  for condition in "${condition_arr[@]}"; do
    # LSTM
    sbatch --array=0-50 cpu_arr_jobsub.cmd "1" "${condition}" "${learn_rate}" "99" "99"
    for alfa in "${alfa_arr[@]}"; do
      for lmda in "${lmda_arr[@]}"; do
        # SEM
        sbatch --array=0-50 cpu_arr_jobsub.cmd "0" "${condition}" "${learn_rate}" "${alfa}" "${lmda}" 
      done
    done
  done
done
